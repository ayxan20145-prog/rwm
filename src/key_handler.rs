...
use crate::{
    ...
    bar,
    config::{KeyBinding, VOLUME_DOWN_CMD, VOLUME_MUTE_CMD, VOLUME_UP_CMD},
    layout,
    workspace::{
    ...
    for binding in bindings {
        if event.detail == binding.key && modifiers_match(event.state, binding.modifiers) {
            match binding.action {
                ...
                Action::Workspace(ws) => switch_workspace(
                    conn,
                    workspaces,
                    current,
                    (ws - 1) as usize,
                ...
                Action::Focus(dir) => match dir {
                    Direction::Left => focus_prev(conn, &workspaces[*current], focused)?,
                    Direction::Right => focus_next(conn, &workspaces[*current], focused)?,
                    Direction::Up => focus_prev(conn, &workspaces[*current], focused)?,
                    Direction::Down => focus_next(conn, &workspaces[*current], focused)?,
                },
